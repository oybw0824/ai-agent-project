...
- **metrics 更新**：`AgentMetrics.sseCompleted` 计数器递增，`sseDuration` 直方图记录耗时
...
### 8.3 监控指标（AgentMetrics）

流式 API 相关的监控指标：

| 指标 | 类型 | 说明 |
|------|------|------|
| `sse.connections` | Counter | SSE 连接数（递增） |
| `sse.completed` | Counter | 正常完成的 SSE 连接数 |
| `sse.timeout` | Counter | SSE 连接超时次数 |
| `sse.error` | Counter | SSE 连接异常次数 |
| `sse.duration` | Histogram | SSE 连接持续时间分布（毫秒） |
| `tool.call.total` | Counter | MCP 工具调用总次数 |
| `tool.call.success` | Counter | MCP 工具调用成功次数 |
| `tool.call.failure` | Counter | MCP 工具调用失败次数 |
| `sse-agent` (gauge) | ThreadPool | SSE 线程池指标（活跃/队列/完成） |

**访问端点**：
- Prometheus：`GET /actuator/prometheus`
- JSON 指标：`GET /actuator/metrics`
- 自定义汇总：`GET /api/v1/metrics`
